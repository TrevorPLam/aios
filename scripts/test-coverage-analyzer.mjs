#!/usr/bin/env node
/**
 * Test Coverage Analyzer for AIOS
 *
 * Note: Bundle analysis not applicable for React Native mobile apps
 */
    ...
    fileStats.percentages = {
      statements:
        fileStats.statements.total > 0
          ? (
              (fileStats.statements.covered / fileStats.statements.total) *
              100
            ).toFixed(2)
          : 100,
      branches:
        fileStats.branches.total > 0
          ? (
              (fileStats.branches.covered / fileStats.branches.total) *
              100
            ).toFixed(2)
          : 100,
      functions:
        fileStats.functions.total > 0
          ? (
              (fileStats.functions.covered / fileStats.functions.total) *
              100
            ).toFixed(2)
          : 100,
      lines:
        fileStats.lines.total > 0
...
function main() {
  ...
  console.log(
    `Statements: ${stats.overall.statements}% (${stats.covered.statements}/${stats.total.statements})`,
  );
  console.log(
    `Branches:   ${stats.overall.branches}% (${stats.covered.branches}/${stats.total.branches})`,
  );
  console.log(
    `Functions:  ${stats.overall.functions}% (${stats.covered.functions}/${stats.total.functions})`,
  );
  console.log(
    `Lines:      ${stats.overall.lines}% (${stats.covered.lines}/${stats.total.lines})`,
  );
  ...
  if (overallAvg >= targetCoverage) {
    ...
    console.log(
      `⚠️  Coverage (${overallAvg.toFixed(2)}%) below target (${targetCoverage}%)`,
    );
    ...
    if (filesNeedingTests.length > 0) {
      ...
      filesNeedingTests.slice(0, 10).forEach((file, index) => {
        ...
        console.log(
          `   Statements: ${file.percentages.statements}%, Branches: ${file.percentages.branches}%, Functions: ${file.percentages.functions}%, Lines: ${file.percentages.lines}%\n`,
        );
      });
    }
