...
function getChangedLines(file) {
  try {
    const output = execSync(`git diff ${BASE_BRANCH}...HEAD -- ${file}`, { encoding: 'utf-8' });
    const addedLines = [];
    let currentLine = 0;
...
function checkNewCodeCoverage() {
  ...
  console.log(`✅ Coverage ${coveragePercent.toFixed(1)}% meets threshold of ${COVERAGE_THRESHOLD}%`);
  return true;
}
