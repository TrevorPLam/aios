...
function generateAgentsToonHeader(targetName, pathType = "global") {
  const agentsToonPath =
    pathType === "global"
      ? "../AGENTS.toon"
      : pathType === "docs"
        ? "../../AGENTS.toon"
  ...
  return `type: agent_directory_guide
id: ${targetName.replace(/\s+/g, "-")}.toon
filepath: ${pathType === "global" ? ".github/AGENTS.toon" : `.github/instructions/${pathType}.AGENTS.toon`}
$schema: http://json-schema.org/draft-07/schema#
version: 1.0.0
purpose: ${targetName} - Auto-generated from constitution.json

parent_governance:
  entry_point: ${agentsToonPath}
  note: Read root AGENTS.toon first for core governance rules.

auto_generated:
  source: .repo/policy/constitution.json
  compiler: scripts/tools/compile-constitution.mjs
  generated_at: ${new Date().toISOString()}
  note: DO NOT EDIT DIRECTLY - edit constitution.json and run npm run compile:constitution

`;
}

function generateGlobalAgentsToon(constitution) {
  const sections = [];

  sections.push("agent_entry_point:\n");
  sections.push("  reference: ../AGENTS.toon\n");
  sections.push("  note: See AGENTS.toon for complete agent workflow, command routing, and decision trees.\n");

  if (constitution.workflow?.three_pass) {
    sections.push("\nworkflow:\n");
    sections.push("  name: Three-pass workflow\n");
    sections.push("  steps[4]:\n");
    sections.push("    Pass 0 (Context): Read folder context files\n");
    sections.push("    Pass 1 (Plan): Determine change type, list actions, identify risks\n");
    sections.push("    Pass 2 (Change): Apply edits following existing patterns\n");
    sections.push("    Pass 3 (Verify): Run tests, provide evidence, update logs\n");
  }

  sections.push("\nrequired_reading:\n");
  sections.push("  order[3]:\n");
  sections.push("    ../AGENTS.toon\n");
...
function generateDocsAgentsToon(constitution) {
  const sections = [];

  sections.push("agent_entry_point:\n");
  sections.push("  reference: ../../AGENTS.toon\n");
  sections.push("  note: See AGENTS.toon for complete agent workflow.\n");

  sections.push("\ndocumentation_context:\n");
  const principle19 = constitution.principles.list.find((p) => p.id === 19);
  if (principle19) {
    sections.push(`  principle_${principle19.id}: ${principle19.rule}\n`);
  }

  return sections.join("");
}

function generateClientAgentsToon(constitution) {
  const sections = [];
  ...
  sections.push("\nclient_context:\n");
  if (constitution.best_practices?.tech_stack?.mobile) {
    sections.push(`  tech_stack: ${constitution.best_practices.tech_stack.mobile}\n`);
  }
  if (constitution.boundaries?.import_direction) {
    sections.push(`  import_direction: ${constitution.boundaries.import_direction}\n`);
  }

  return sections.join("");
}

function generateServerAgentsToon(constitution) {
  const sections = [];
  ...
  sections.push("\nserver_context:\n");
  if (constitution.best_practices?.tech_stack?.backend) {
    sections.push(`  tech_stack: ${constitution.best_practices.tech_stack.backend}\n`);
  }
  if (constitution.security?.review_triggers) {
    sections.push("\nsecurity_review_triggers:\n");
    sections.push("  note: The following changes require security review (HITL)\n");
    const triggers = Object.entries(constitution.security.review_triggers.meanings);
    sections.push(`  triggers[${triggers.length}]:\n`);
    triggers.forEach(([id, meaning]) => {
      sections.push(`    ${id}: ${meaning}\n`);
    });
  }
...
function compileConstitution() {
  ...
  // Generate Global AGENTS.toon
  const globalAgentsContent =
    generateAgentsToonHeader("Global Agent Instructions", "global") +
    generateGlobalAgentsToon(constitution);
  const globalAgentsPath = join(OUTPUT_DIR, "AGENTS.toon");
  ...
  // Generate Docs AGENTS.toon
  const docsAgentsContent =
    generateAgentsToonHeader("Documentation Agent Instructions", "docs") +
    generateDocsAgentsToon(constitution);
  const docsAgentsPath = join(INSTRUCTIONS_DIR, "docs.AGENTS.toon");
  ...
  // Generate Client AGENTS.toon
  const clientAgentsContent =
    generateAgentsToonHeader("Client/Frontend Agent Instructions", "client") +
    generateClientAgentsToon(constitution);
  const clientAgentsPath = join(INSTRUCTIONS_DIR, "client.AGENTS.toon");
  ...
  // Generate Server AGENTS.toon
  const serverAgentsContent =
    generateAgentsToonHeader("Server/Backend Agent Instructions", "server") +
    generateServerAgentsToon(constitution);
  const serverAgentsPath = join(INSTRUCTIONS_DIR, "server.AGENTS.toon");
  writeFileSync(serverAgentsPath, serverAgentsContent, "utf-8");
  console.log(`Generated: ${serverAgentsPath}`);

  console.log("\nSummary:");
  console.log(
    "   Generated: 8 files (4 instruction files + 4 AGENTS.toon files)",
  );
