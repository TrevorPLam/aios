...
5. **Voice Transcription** - Voice-to-note (mobile advantage)
6. **Smart Tag Suggestions** - AI recommends tags (unique)
7. **Reading Time Estimation** - How long to read (Bear has this)
8. **Markdown Preview Mode** - Side-by-side edit/preview (Obsidian)

**Future (Advanced Features):**
9. **Version History** - Track changes over time (Notion, Google Docs)
10. **Collaborative Editing** - Real-time co-editing (Notion)
...
4. Add tag validation (trim, non-empty check)
5. Fix array mutation in getSorted (use spread operator)
6. Extract word count calculation to helper method
7. Add debug logging for not-found cases

### Priority 3 (Nice-to-Have - Do Eventually)

8. Extract Jaccard similarity to utility module
9. Add performance guard for very large notes in findSimilar
10. Enhance module header with architecture notes and usage examples
