...
|---|-----------|-------------------------|------------------|----------|--------|--------|-----------|
| 1 | `apps/api/__tests__/api.e2e.test.ts` - auth workflow | User passwords must be hashed before storage | Removed `bcrypt.hash()` - stores plaintext password | FAIL | ✅ PASS | ❌ **FALSE NEGATIVE** | Test doesn't verify password hashing, only checks token generation works |
| 2 | `apps/api/__tests__/analytics.test.ts` - idempotency | Duplicate events must be skipped | Removed duplicate check in `saveAnalyticsEvents()` | FAIL | ✅ PASS | ❌ **FALSE NEGATIVE** | Test passes because `Map.set()` overwrites, test doesn't verify skip behavior |
| 3 | `apps/mobile/storage/__tests__/tasks.test.ts` - bulk update | Task status must update correctly | Removed `status` field from bulk update | FAIL | ❌ FAIL | ✅ CORRECT | Test properly checks status field |
| 4 | `apps/mobile/lib/__tests__/memoryManager.test.ts` - access tracking | Access count must increment on repeated calls | Removed `+1` from accessCount increment | FAIL | ✅ PASS | ⚠️ **WEAK TEST** | Uses `toBeGreaterThanOrEqual(2)` which passes with initial count of 1 |
| 5 | `apps/mobile/lib/__tests__/attentionManager.test.ts` - focus mode | Focus mode must filter non-urgent items | Bypassed all focus mode filtering logic | FAIL | ❌ FAIL | ✅ CORRECT | Test properly validates filtering behavior |
| 6 | `apps/mobile/__tests__/integrationsScreen.test.tsx` - navigation | User must navigate to detail screen on tap | Removed `navigation.navigate()` call | FAIL | ❌ FAIL | ✅ CORRECT | Mock properly catches missing navigation |
| 7 | `apps/mobile/components/__tests__/Button.test.tsx` - disabled state | Disabled buttons must not trigger onPress | Removed `disabled` prop from Pressable | FAIL | ❌ FAIL | ✅ CORRECT | Test catches disabled state bug |
...
1. **CRITICAL: Password Hashing Not Verified**
   - **Location:** `apps/api/__tests__/api.e2e.test.ts:39-105`
...
2. **Analytics Idempotency Test False Positive**
   - **Location:** `apps/api/__tests__/analytics.test.ts:69-86`
...
3. **Weak Access Count Assertions**
   - **Location:** `apps/mobile/lib/__tests__/memoryManager.test.ts:154-163`
...
4. **Over-Mocked Navigation Tests**
   - **Location:** `apps/mobile/__tests__/integrationsScreen.test.tsx:6-18`
...
5. **Database Mock Inconsistency**
   - **Location:** `apps/mobile/__tests__/integrationsScreen.test.tsx:41-48`
...
7. **Missing Async Error Handling Tests**
   - **Location:** `apps/mobile/storage/__tests__/database.test.ts` (various CRUD tests)
...
8. **Statistics Test Using Time-Dependent Logic**
   - **Location:** `apps/mobile/storage/__tests__/tasks.test.ts:333-372`
...
9. **Search Test Order Dependency**
   - **Location:** `apps/mobile/lib/__tests__/searchIndex.test.ts:312-315`
...
10. **No Mutation/Integration Tests for Auth Middleware**
    - **Location:** `apps/api/middleware/auth.ts` (no dedicated test file)
...
- **File:** `apps/mobile/lib/contextEngine.ts:175-184`
...
1. **Add Password Hashing Verification Test**
   ```typescript
   // In apps/api/__tests__/api.e2e.test.ts
...
2. **Fix Analytics Idempotency Test**
   ```typescript
   // In apps/api/__tests__/analytics.test.ts:69-86
...
**Evidence:**
```typescript
// Current test (apps/api/__tests__/api.e2e.test.ts:39-92)
...
**Evidence:**
```typescript
// Current test (apps/api/__tests__/analytics.test.ts:69-86)
await storage.saveAnalyticsEvents([event]);
await storage.saveAnalyticsEvents([event]); // Duplicate
...
**Mutation Applied:**
```typescript
// In apps/api/storage.ts:766-771
// BEFORE (correct):
