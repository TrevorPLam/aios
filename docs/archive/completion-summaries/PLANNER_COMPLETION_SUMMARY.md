...
10. `search(query)` - Search by title/notes
11. `getByStatus(status)` - Filter by status
12. `getByPriority(priority)` - Filter by priority
13. `getOverdue()` - Get overdue tasks
14. `getDueToday()` - Get tasks due today
15. `getDueInDays(days)` - Get tasks due soon
