# AIOS Documentation Index

**Last Updated:** January 21, 2026

## Post-Consolidation Structure

This index provides direct links to all active documentation in the repository.

---

## 🏠 Root Documentation (Entry Points)

| Document | Purpose | Size |
| ---------- | --------- | ------ |
| [README.md](../README.md) | Project overview, setup, quick start | 560 lines |
| [GOVERNANCE.md](../GOVERNANCE.md) | Governance model and decision process | - |
| [CONTRIBUTING.md](../CONTRIBUTING.md) | Contribution guidelines | - |
| [CODE_OF_CONDUCT.md](../CODE_OF_CONDUCT.md) | Community standards and enforcement | - |
| [SECURITY.md](../SECURITY.md) | Security policy & reporting | 70 lines |
| [CHANGELOG.md](../CHANGELOG.md) | Release history and notable changes | - |
| [LICENSE](../LICENSE) | Project license | - |
| [BESTPR.md](../BESTPR.md) | Agent best practices and workflow guidance | - |
| [F&F.md](../F&F.md) | Features & Functionality (main reference) | 669 lines |
| [MODULE_DETAILS.md](../MODULE_DETAILS.md) | Technical implementation details | 469 lines |
| [DOCUMENTATION_GUIDE.md](../DOCUMENTATION_GUIDE.md) | Navigation guide (start here!) | 200+ lines |
| [QUICK_REFERENCE.md](../QUICK_REFERENCE.md) | Mobile configuration reference | 145 lines |
| [DOCUMENTATION_METRICS.md](../DOCUMENTATION_METRICS.md) | Documentation health dashboard | - |

---

## ✅ Task Tracking (Source of Truth)

- [P0TODO.md](../P0TODO.md) - P0 critical tasks
- [P1TODO.md](../P1TODO.md) - P1 high-impact tasks
- [P2TODO.md](../P2TODO.md) - P2 medium-priority tasks
- [P3TODO.md](../P3TODO.md) - P3 backlog tasks

---

## 📁 Technical Documentation

### Implementation & Setup

- [API_DOCUMENTATION.md](technical/API_DOCUMENTATION.md) - Complete API reference
- [TESTING_INSTRUCTIONS.md](technical/TESTING_INSTRUCTIONS.md) - Testing guide
- [IMPLEMENTATION_ROADMAP.md](technical/IMPLEMENTATION_ROADMAP.md) - Development roadmap
- [MOBILE_CONFIGURATION_EXPLANATION.md](technical/MOBILE_CONFIGURATION_EXPLANATION.md) - Mobile config details
- [lib-usage.md](technical/lib-usage.md) - Client lib usage examples

### Design & Architecture

- [design_guidelines.md](technical/design_guidelines.md) - UI/UX design system
- [CONTEXTUAL_NAVIGATION.md](technical/CONTEXTUAL_NAVIGATION.md) - Navigation patterns
- [NAVIGATION_IMPROVEMENTS.md](technical/NAVIGATION_IMPROVEMENTS.md) - Navigation enhancements
- [STYLE_GUIDE.md](STYLE_GUIDE.md) - Documentation style guide
- [architecture/diagrams/](architecture/diagrams/) - Architecture diagrams (Mermaid-based)
  - [System Architecture](architecture/diagrams/system-architecture.md)
  - [Data Flow](architecture/diagrams/data-flow.md)
  - [Module Relationships](architecture/diagrams/module-relationships.md)
  - [Deployment Architecture](architecture/diagrams/deployment-architecture.md)

### Decisions & Governance

- [decisions/](decisions/) - Architecture decision records
- [governance/constitution.md](governance/constitution.md) - Constitution (highest authority)
- [governance/state.md](governance/state.md) - Living state & enforcement toggles
- [governance/AIG.md](governance/AIG.md) - Governance & AI index
- [.templates/](.templates/) - Documentation templates
- [processes/](processes/) - Documentation processes and workflows
  - [Feedback Loop](processes/feedback-loop.md) - User feedback collection and review process
  - [DOCUMENTATION_BEST_PRACTICES.md](processes/DOCUMENTATION_BEST_PRACTICES.md) - Documentation standards
  - [DOCUMENTATION_QUALITY_CONTROL.md](processes/DOCUMENTATION_QUALITY_CONTROL.md) - Documentation QA review

### Deployment

- [replit-deployment.md](technical/replit-deployment.md) - Replit deployment guide

---

## 🔒 Security Documentation

- [SECURITY.md](security/SECURITY.md) - Comprehensive security documentation
  - General security overview
  - Module-specific security status (all 14 modules)
  - Authentication & authorization
  - Input validation
  - Data protection
  - Security best practices
  - Audit history

---

## 📊 Analysis Documentation

- [COMPETITIVE_ANALYSIS.md](analysis/COMPETITIVE_ANALYSIS.md) - Market positioning
  - Command Center vs Notion AI, Motion, Superhuman
  - Notebook vs Notion, Obsidian, Bear
  - Lists vs Todoist, Any.do
  - Budget vs YNAB, Mint
  - Calendar vs Fantastical, Google Calendar
  - Feature comparison matrices

- [CODE_QUALITY_ANALYSIS.md](analysis/CODE_QUALITY_ANALYSIS.md) - Code quality reports
  - General code quality analysis
  - Email module specific analysis
  - Quality metrics and recommendations

---

## 📅 Planning Documentation

- [MISSING_FEATURES.md](planning/MISSING_FEATURES.md) - Roadmap & known limitations
  - Critical missing features
  - Backend API gaps
  - Module-specific limitations
  - Future enhancements

---

## 📈 Analytics Documentation

- [WORLD_CLASS_ANALYTICS_ROADMAP.md](analytics/WORLD_CLASS_ANALYTICS_ROADMAP.md) - Analytics strategy
- [WORLD_CLASS_FEATURES_SUMMARY.md](analytics/WORLD_CLASS_FEATURES_SUMMARY.md) - Feature benchmarks
- [telemetry.md](analytics/telemetry.md) - Telemetry implementation

---

## 🗄️ Archive (Historical Documents)

### Pre-Consolidation

- [2026-01-pre-consolidation/](archive/2026-01-pre-consolidation/)
  - F&F-BACKUP.md (3,719 lines - original before restructure)
  - TASK_COMPLETION_SUMMARY_OLD.md

### Module Completion Summaries (10 files)

- [completion-summaries/](archive/completion-summaries/)
  - ALERTS_MODULE_COMPLETION_SUMMARY.md
  - BUDGET_MODULE_COMPLETION_SUMMARY.md
  - CALENDAR_MODULE_COMPLETION_SUMMARY.md
  - COMMAND_CENTER_COMPLETION_SUMMARY.md
  - EMAIL_MODULE_COMPLETION_SUMMARY.md
  - LISTS_MODULE_COMPLETION_SUMMARY.md
  - NOTEBOOK_MODULE_COMPLETION_SUMMARY.md
  - PHOTOS_MODULE_COMPLETION_SUMMARY.md
  - PLANNER_COMPLETION_SUMMARY.md
  - TRANSLATOR_MODULE_COMPLETION_SUMMARY.md

### Enhancement Summaries (11 files)

- [enhancements/](archive/enhancements/)
  - ALERTS_ENHANCEMENT_SUMMARY.md
  - BUDGET_MODULE_ENHANCEMENTS.md
  - CONTACTS_MODULE_ENHANCEMENTS.md
  - EMAIL_MODULE_ENHANCEMENTS.md
  - HISTORY_MODULE_ENHANCEMENTS.md
  - INTEGRATIONS_MODULE_ENHANCEMENTS.md
  - LISTS_ENHANCEMENT_SUMMARY.md
  - NOTEBOOK_ENHANCEMENTS.md
  - PHOTOS_MODULE_ENHANCEMENTS.md
  - PLANNER_MODULE_ENHANCEMENTS.md
  - TRANSLATOR_ENHANCEMENT_SUMMARY.md

### Analysis Reports (36+ files)

- [analysis/](archive/analysis/)
  - High-level analyses (6 files)
  - Perfect codebase analyses (6 files)
  - Final analysis reports (3 files)
  - Other analysis documents (3 files)
  - [forensic/](archive/analysis/forensic/) - Forensic analysis summaries

### Security Summaries (9 files)

- [security/](archive/security/)
  - Module-specific security summaries
  - Historical security analyses

### Project Management (11 files)

- [project-management/](archive/project-management/)
  - Task completion summaries
  - Implementation summaries
  - Session summaries

### Decisions (Archived)

- [decisions/](archive/decisions/)
  - ADR rollups superseded by `docs/decisions/`

### Sessions

- [sessions/](archive/sessions/)
  - Session reports and completion summaries

---

## 🔍 Quick Navigation by Topic

### For Developers

1. Start: [README.md](../README.md)
2. Setup: [README.md](../README.md#️-installation)
3. API: [docs/technical/API_DOCUMENTATION.md](technical/API_DOCUMENTATION.md)
4. Testing: [docs/technical/TESTING_INSTRUCTIONS.md](technical/TESTING_INSTRUCTIONS.md)
5. Contributing: [CONTRIBUTING.md](../CONTRIBUTING.md)

### For Product Managers

1. Features: [F&F.md](../F&F.md)
2. Roadmap: [docs/planning/MISSING_FEATURES.md](planning/MISSING_FEATURES.md)
3. Competitive Position: [docs/analysis/COMPETITIVE_ANALYSIS.md](analysis/COMPETITIVE_ANALYSIS.md)
4. Module Status: [F&F.md](../F&F.md) (Quick Reference table)

### For Security Reviewers

1. Security Policy: [SECURITY.md](../SECURITY.md)
2. Detailed Security: [docs/security/SECURITY.md](security/SECURITY.md)
3. Module Security Status: [docs/security/SECURITY.md](security/SECURITY.md#module-security-status)
4. Reporting: [SECURITY.md](../SECURITY.md#reporting-a-vulnerability)

### For Designers

1. Design System: [docs/technical/design_guidelines.md](technical/design_guidelines.md)
2. Navigation: [docs/technical/CONTEXTUAL_NAVIGATION.md](technical/CONTEXTUAL_NAVIGATION.md)
3. UI Improvements: [docs/technical/NAVIGATION_IMPROVEMENTS.md](technical/NAVIGATION_IMPROVEMENTS.md)

### For Researchers

1. Competitive Analysis: [docs/analysis/COMPETITIVE_ANALYSIS.md](analysis/COMPETITIVE_ANALYSIS.md)
2. Code Quality: [docs/analysis/CODE_QUALITY_ANALYSIS.md](analysis/CODE_QUALITY_ANALYSIS.md)
3. Analytics Strategy: [docs/analytics/WORLD_CLASS_ANALYTICS_ROADMAP.md](analytics/WORLD_CLASS_ANALYTICS_ROADMAP.md)
4. Historical Data: [docs/archive/](archive/)

---

## 📊 Statistics

### Active Documentation

- **Root Files:** 10
- **Technical Docs:** 8
- **Security Docs:** 1 (consolidated)
- **Analysis Docs:** 2
- **Planning Docs:** 1
- **Analytics Docs:** 3
- **Total Active:** ~25 files

### Archived Documentation

- **Pre-consolidation:** 2 files
- **Completion Summaries:** 10 files
- **Enhancements:** 11 files
- **Analysis:** 18 files
- **Security:** 9 files
- **Project Management:** 5 files
- **Total Archived:** 55 files

### Reduction Metrics

- **Root Directory:** 72 → 10 files (86% reduction)
- **Total Docs:** 79 → 25 files (68% reduction)
- **Duplication:** Eliminated (single source of truth established)

---

## 🎯 Single Source of Truth

| Information Type | Primary Source | Secondary Source |
| ----------------- | ---------------- | ------------------ |
| Module Status | [F&F.md](../F&F.md) | [MODULE_DETAILS.md](../MODULE_DETAILS.md) |
| Technical Details | [MODULE_DETAILS.md](../MODULE_DETAILS.md) | [docs/technical/](technical/) |
| API Reference | [docs/technical/API_DOCUMENTATION.md](technical/API_DOCUMENTATION.md) | - |
| Security Status | [docs/security/SECURITY.md](security/SECURITY.md) | [SECURITY.md](../SECURITY.md) |
| Missing Features | [docs/planning/MISSING_FEATURES.md](planning/MISSING_FEATURES.md) | - |
| Competitive Position | [docs/analysis/COMPETITIVE_ANALYSIS.md](analysis/COMPETITIVE_ANALYSIS.md) | [F&F.md](../F&F.md) |

---

## 🔗 External Links

- **GitHub Repository:** [TrevorPLam/aios](https://github.com/TrevorPLam/aios)
- **Issues:** [GitHub Issues](https://github.com/TrevorPLam/aios/issues)
- **Pull Requests:** [GitHub PRs](https://github.com/TrevorPLam/aios/pulls)

---

**Consolidation Date:** January 16, 2026
**Maintained By:** Documentation Team
**Version:** 3.0 (Post-Consolidation)
