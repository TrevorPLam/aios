...
**Version:** 1.0  
**Status:** Active  
**Authority:** HIGHEST - This is the single source of truth for all repository governance
...
|---------|-----|-----|---------|------|---------|-------|----------|------------|
| User Auth | TODO | ADR-003 | server/middleware/auth.ts | /api/login | users table | server/__tests__/auth.test.ts | ops/runbooks/auth-issues.md | TODO |
```
...
- `server/middleware/auth*` → @security-team
