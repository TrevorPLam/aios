    ...
    try:
        if file_path.exists():
            ...
            if "## Current Task" in content:
                # Insert after "## Current Task" section
                lines = content.split("\n")
                insert_pos = next((i for i, line in enumerate(lines) if line.strip() == "## Current Task"), len(lines))
...
def main():
    ...
    parser.add_argument("--backlog-file", type=str, default="agents/tasks/BACKLOG.md", help="Backlog file path")
    parser.add_argument("--todo-file", type=str, default="agents/tasks/TODO.md", help="TODO file path")
    ...
    # Check if TODO already has a task
    if todo_path.exists():
        todo_content = todo_path.read_text(encoding="utf-8")
        if "TASK-" in todo_content and not todo_content.count("TASK-") >= 2:
            print("⚠️  TODO.md already contains a task. Only one task should be active.")
