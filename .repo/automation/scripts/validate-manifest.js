...
function main() {
  ...
  console.log(`📦 Found ${Object.keys(manifestCommands).length} commands in manifest\n`);
  ...
  for (const [name, command] of Object.entries(manifestCommands)) {
    ...
    if (command === "<FILL_FROM_REPO>") {
      warnings.push(`Command '${name}' is marked as <FILL_FROM_REPO> - needs implementation`);
      continue;
    }
  ...
  const importantPackageScripts = ["check:types", "check:format", "lint", "test"];
